...
```{r}
ex <- read_exif('resources/videos/2023/dunlins/12-09-2023_dunlin3206-2.MOV')
```
...
preprocess_observation <- function(observation_path) {
  ...
  # if the last observation was a camera malfunction, just clip the behaviour 
  # early, this is often assigned after the bird flies away and is out of sight
...
get_watlas_for_observation <- function(observation) {
  watlas <- read.csv(glue(
    'resources/watlas/dunlin-2022-2023/dunlin-{tag}.csv',
    tag = observation$tag
  ))
  watlas$time <- ymd_hms(watlas$time, tz = "UTC")
  watlas$time <- with_tz(watlas$time, tzone = "Europe/Amsterdam")
  watlas_interval <-
    interval(watlas[1, "time"], watlas[nrow(watlas), "time"])
  ...
  # create turning angle feature
  watlas$angle <- atl_turning_angle(data = watlas,
                                    time = "TIME",
                                    X = "X_raw",
                                    Y = "Y_raw")
  ...
  watlas$dtime <- sapply(
    ...
    FUN = function(i) {
      if (i == 1) return(0)
      return(watlas[i, "time"] - watlas[i - 1, "time"])
    }
  )
  ...
  # link a behaviour to a watlas localisation
  watlas$behaviour <- sapply(
    1:nrow(watlas),
    FUN = function(i) {
      ...
      # find what behaviour (if any) has been observed since the last localisation
      x_entries <-
      ...
      # TODO feedback on this
      if ("flying" %in% x_entries$Behavior) return("flying")
      ...
      if (nrow(x_entries) > 0) {
        x_behaviours <- c("active" = 0, "inactive" = 0, "camera malfunction" = 0)
        ...
        for (j in 1:nrow(x_entries)) {
          ...
          x_behaviours[x_entries[j, "Behavior"]] <-
            x_behaviours[[x_entries[j, "Behavior"]]] + secs
        }
        # return the behavior with the most seconds
        most_frequent_behaviour <- names(x_behaviours[x_behaviours == max(x_behaviours)])
        return(most_frequent_behaviour)
      
      # else if there was no observation since the last localisation, that means
      # that we're in the middle of a BORIS START - STOP interval, and the 
      # observation STOP is after this localisation
      } else {
        # find the first following observed behaviour relative to this localisation 
        x <- watlas[i, "time"] - observation$data$Time
        closest <- min(abs(x[x <= 0]))
        closest_observation <-
          observation$data[which(x == -closest), ][1,]
        return(closest_observation$Behavior)
      }
    }
  )
  return(watlas)
